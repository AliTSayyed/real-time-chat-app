...
export class CustomDatePipe implements PipeTransform {
  ...
  transform(value: string): string | null {
    ...
    if (this.isSameDay(currentDate, messageDate)){
      return 'Today ' + datePipe.transform(messageDate, 'shortTime');
    }

    // Return Yesterday hh : mm if message was sent one day ago
    if (this.isYesterday(currentDate, messageDate)){
      return 'Yesterday ' + datePipe.transform(messageDate, 'shortTime');
    }
    ...
    if (timeDifference >= 2 * oneDay && timeDifference < 6 * oneDay ) {
      return datePipe.transform(messageDate, 'EEE h:mm a'); // Day of the week, hour : minute, 
    }

    // else reutrn the date as month, day, year, time. 
    return datePipe.transform(messageDate, 'MM/dd/yy h:mm a');
  } 
